        ...
        if self.iteration % self.it_per_evaluation == 0:
                if self.iteration > 0:
                    ...
                    if self.current_evaluation >= self.nb_evaluations:
                        print ("\tenfant params        =",self.param)
                        print ("\tenfant score         =",self.total_score)
                        
                        # Sauvegarder dans le fichier (score du parent actuel)
                        if self.results_file:
                            self.results_file.write(f"{self.evaluation_number},{self.parent_score},{self.best_score}\n")
                            self.results_file.flush()
                        self.evaluation_number += 1
                        
                        # ALGO GENETIQUE (mu=1, lambda=1)
                        # Si enfant meilleur que parent → enfant devient le nouveau parent
                        if self.total_score > self.parent_score:
                            self.parent_param = self.param.copy()
                            self.parent_score = self.total_score
                            print ("[ENFANT ACCEPTE] devient parent")
                        else:
                            print ("[PARENT CONSERVE] enfant rejeté")

                        # Sauvegarder le meilleur global
                        ...
                        if self.mode == "search":
                            if self.trial >= self.max_trials:
                                ...
                                # Créer un enfant par mutation du parent
                                self.param = self._mutate(self.parent_param)
                                self.trial = self.trial + 1
                                print ("Generation",self.trial,": mutation")
